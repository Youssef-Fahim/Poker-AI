...
class LookUpTable:
    ...
    def feed_flush(self):
        ...
        flushes = []
        init_gen = self.get_lexographically_next_bit_sequence(int('0b0000000000011111', 2))
    ...
    def feed_unique(self):
        ...
        """
        for straight_, rank in zip(self.straight, list(range(LookUpTable.MAX_RANK_FLUSH + 1, LookUpTable.MAX_RANK_STRAIGHT + 1))):
            prime_product = Card.get_prime_product_from_rankbits(straight_)
            self.lookup_table_unique[prime_product] = rank

        for nstraight_, rank in zip(self.not_straight, list(range(LookUpTable.MAX_RANK_ONE_PAIR + 1, LookUpTable.MAX_RANK_HIGH + 1))):
